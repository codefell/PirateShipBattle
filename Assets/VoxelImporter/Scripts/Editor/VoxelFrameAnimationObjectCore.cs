...
namespace VoxelImporter
{
    public class VoxelFrameAnimationObjectCore : VoxelBaseCore
    {
                ...
                for (int i = 0; i < voxelObject.frames.Count; i++)
                {
                    if (voxelObject.frames[i].voxelFileObject != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(assetPath))
                        {
                            ...
                            voxelObject.frames[i].voxelFilePath = fullPath;
                        }
                    }
                }
            }
        ...
        public override void Reset(string path, UnityEngine.Object obj)
        {
            ...
            else if(GetFileType(path) == VoxelBase.FileType.vox)
            {
                ...
                if (subCount > 1)
                {
                    ...
                    {
                        voxelObject.frames.Add(new VoxelFrameAnimationObject.FrameData() { voxelFilePath = path, voxelFileObject = obj, voxelFileSubIndex = i });
                    }
                    done = true;
                }
            }
        ...
        public override bool ReadyVoxelData(bool forceReload = false)
        {
            if (forceReload)
            {
                ...
                for (int i = 0; i < voxelObject.frames.Count; i++)
                {
                    voxelObject.frames[i].voxelData = null;
                }
            }
        ...
        protected override bool LoadVoxelData()
        {
            ...
            if(voxelObject.frames != null && voxelObject.frames.Count > 0)
            {
                ...
                voxelObject.voxelFileSubIndex = voxelObject.frames[0].voxelFileSubIndex;
        ...
        protected override bool CreateMesh()
        {
            ...
            #region Combine VoxelData
            {
                ...
                for (int i = 0; i < voxelObject.frames.Count; i++)
                {
                    ...
                    chunkDataList[i].area = new VoxelData.ChunkArea() { min = new IntVector3(0, 0, offset), max = new IntVector3(voxelObject.frames[i].voxelData.voxelSize.x, voxelObject.frames[i].voxelData.voxelSize.y, offset + voxelObject.frames[i].voxelData.voxelSize.z) };
