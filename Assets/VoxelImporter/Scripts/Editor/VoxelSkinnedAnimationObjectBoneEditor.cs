                        ...
                        editorBoneEditorWindowRect = GUILayout.Window(EditorGUIUtility.GetControlID(FocusType.Passive, editorBoneEditorWindowRect), editorBoneEditorWindowRect, (id) =>
                        {
                            ...
                            if (!objectTarget.edit_helpEnable)
                            {
                                #region "?"
                                {
                                    EditorGUILayout.BeginHorizontal();
                                    EditorGUILayout.Space();
                                    if (GUILayout.Button("?", objectTarget.edit_helpEnable ? editorCommon.guiStyleActiveButton : GUI.skin.button, GUILayout.Width(16)))
                                    {
                ...
                case VoxelSkinnedAnimationObject.Edit_VoxelWeightMode.Voxel:
                    {
                        ...
                        if (result.HasValue)
                        {
                            editorCommon.cursorMesh = objectCore.Edit_CreateMesh(new List<VoxelData.Voxel>() { new VoxelData.Voxel() { position = result.Value, palette = -1 } });
                        }
                    }
                    break;
