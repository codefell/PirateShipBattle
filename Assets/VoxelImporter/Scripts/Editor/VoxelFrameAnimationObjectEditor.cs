    ...
    public class VoxelFrameAnimationObjectEditor : VoxelBaseEditor
    {
                                    ...
                                    if (obj != null)
                                    {
                                        ...
                                        else if (objectCore.GetFileType(path) == VoxelBase.FileType.vox)
                                        {
                                            ...
                                            if (subCount > 1)
                                            {
                                                ...
                                                {
                                                    objectTarget.frames.Add(new VoxelFrameAnimationObject.FrameData() { voxelFilePath = path, voxelFileObject = obj, voxelFileSubIndex = i });
                                                }
                                                done = true;
                                            }
                                        }
                                ...
                                else
                                {
                                    ...
                                    if (objectCore.GetFileType(path) == VoxelBase.FileType.vox)
                                    {
                                        ...
                                        if (subCount > 1)
                                        {
                                            for (int i = 0; i < subCount; i++)
                                            {
                                                objectTarget.frames.Add(new VoxelFrameAnimationObject.FrameData() { voxelFilePath = path, voxelFileSubIndex = i });
                                            }
                                            done = true;
                                        }
                                    }
                                            ...
                                            for (int i = 0; i < DragAndDrop.objectReferences.Length; i++)
                                            {
                                                ...
                                                else if (objectCore.GetFileType(path) == VoxelBase.FileType.vox)
                                                {
                                                    ...
                                                    if (subCount > 1)
                                                    {
                                                        ...
                                                        {
                                                            objectTarget.frames.Add(new VoxelFrameAnimationObject.FrameData() { voxelFilePath = path, voxelFileObject = obj, voxelFileSubIndex = j });
                                                        }
                                                        done = true;
                                                    }
                                                }
                                            ...
                                            for (int i = 0; i < DragAndDrop.paths.Length; i++)
                                            {
                                                ...
                                                if (objectCore.GetFileType(path) == VoxelBase.FileType.vox)
                                                {
                                                    ...
                                                    if (subCount > 1)
                                                    {
                                                        for (int j = 0; j < subCount; j++)
                                                        {
                                                            objectTarget.frames.Add(new VoxelFrameAnimationObject.FrameData() { voxelFilePath = path, voxelFileSubIndex = j });
                                                        }
                                                        done = true;
                                                    }
                                                }
                    ...
                    {
                        {
                            ...
                            if (helpList.Count > 0)
                            {
                                EditorGUILayout.HelpBox(GetHelpStrings(new List<string>(helpList)), MessageType.Error);
                                EditorGUILayout.BeginHorizontal();
                                EditorGUILayout.Space();
            ...
            frameList.drawElementCallback = (rect, index, isActive, isFocused) =>
            {
                ...
                if (index < objectTarget.frames.Count && objectTarget.frames[index] != null)
                {
                    ...
                    #region Object
                    {
                        ...
                        {
                            EditorGUI.BeginChangeCheck();
                            var obj = EditorGUI.ObjectField(r, objectTarget.frames[index].voxelFileObject, typeof(UnityEngine.Object), false);
                            if (EditorGUI.EndChangeCheck())
                            {
                                ...
                                if (baseCore.IsEnableFile(path))
                                {
                                    ...
                                    objectTarget.frames[index].voxelData = null;
                                    objectTarget.frames[index].voxelDataCreatedVoxelFileTimeTicks = 0;
                                    Refresh();
                                }
                            }
                        }
        ...
        private static void ContextSaveAllUnsavedAssets(MenuCommand menuCommand)
        {
            ...
            if (objectTarget.frames != null)
            {
                for (int i = 0; i < objectTarget.frames.Count; i++)
                {
                    if (objectTarget.frames[i].mesh != null && !IsMainAsset(objectTarget.frames[i].mesh))
                    {
