...
namespace VoxelImporter
{
    public abstract class VoxelBaseCore
    {
        ...
        public virtual bool ReadyVoxelData(bool forceReload = false)
        {
            if (forceReload)
                voxelBase.voxelData = null;
            if (voxelBase.voxelData == null)
        ...
        protected virtual bool LoadVoxelData()
        {
            ...
            if(voxelBase.voxelData != null && voxelBase.voxelData.materials == null)
            {
        ...
        protected bool LoadVoxelDataFromVOX(BinaryReader br)
        {
            ...
            #endregion
            List<IntVector3> allVoxelSize = new List<IntVector3>();
            List<VoxelData.Voxel[]> allVoxels = new List<VoxelData.Voxel[]>();
            for (int modelIndex = 0; ; modelIndex++)
            {
                IntVector3 size = IntVector3.zero;
                VoxelData.Voxel[] data = null;
                #region SIZE
                if (!SeekChunk("SIZE"))
                {
                    if (modelIndex == 0)
                ...
                data = new VoxelData.Voxel[numVoxels];
                for (int i = 0; i < numVoxels; i++)
                {
                    ...
                    data[i] = new VoxelData.Voxel(x, y, z, index - 1);
                }
                #endregion
                allVoxelSize.Add(size);
                allVoxels.Add(data);
            }
            ...
            #endregion
            #region Material
            Dictionary<int, VoxelData.VoxMaterial> materials = null;
            if (SeekChunk("MATT"))   //0.98 Only?
            {
                ...
                while (SeekChunk("MATT"))
                {
                    ...
                    if (materialType == VoxelData.VoxMaterial.Type.diffuse)
                        continue;
                    if (materials == null)
                        materials = new Dictionary<int, VoxelData.VoxMaterial>();
                    materials.Add(index, new VoxelData.VoxMaterial()
                    {
                ...
                while (SeekChunk("MATL"))
                {
                    ...
                    for (int i = 0; i < valueCount; i++)
                    {
                        ...
                        case "_type":
                            ...
                            break;
                        case "_weight":
                            materialWeight = Convert.ToSingle(param);
                            break;
                        case "_plastic":
                            normalizedPropertyValues[0] = Convert.ToSingle(param);  //| bit(0) : Plastic
                            break;
                        case "_rough":
                            normalizedPropertyValues[1] = Convert.ToSingle(param);  //| bit(1) : Roughness
                            break;
                        case "_spec":
                            normalizedPropertyValues[2] = Convert.ToSingle(param);  //| bit(2) : Specular
                            break;
                        case "_ior":
                            normalizedPropertyValues[3] = Convert.ToSingle(param);  //| bit(3) : IOR
                            break;
                        case "_att":
                            normalizedPropertyValues[4] = Convert.ToSingle(param);  //| bit(4) : Attenuation
                            break;
                        case "_flux":
                            normalizedPropertyValues[5] = Convert.ToSingle(param);  //| bit(5) : Power
                            break;
                        case "_glow":
                            normalizedPropertyValues[6] = Convert.ToSingle(param);  //| bit(6) : Glow
                            break;
                        case "_unit":
                            normalizedPropertyValues[7] = Convert.ToSingle(param);  //| bit(7) : isTotalPower (*no value)
                            break;
                        }
                    }
                    if (materialType == VoxelData.VoxMaterial.Type.diffuse)
            ...
            #region compress palette
            {
                ...
                Dictionary<int, VoxelData.VoxMaterial> materialList = null;
                for (int i = 0; i < paletteCount.Length; i++)
                {
                    if (paletteCount[i] > 0)
                    {
                        if(materials != null && materials.ContainsKey(i))
                        {
                            if (materialList == null)
                                materialList = new Dictionary<int, VoxelData.VoxMaterial>();
                            materialList.Add(paletteList.Count, materials[i]);
                        }
            ...
            if (voxelBase.loadFromVoxelFile && materials != null)
            {
                ...
                {
                    Color32 color32 = palettes[pair.Key];
                    var materialData = new MaterialData()
                    {
                        name = string.Format("{0} {1} {2}", color32.r, color32.g, color32.b),
                        transparent = pair.Value.materialType == VoxelData.VoxMaterial.Type.glass,
                    };
                    switch (pair.Value.materialType)
                    {
                    ...
                    case VoxelData.VoxMaterial.Type.metal:
                        materialData.material.renderingMode = MaterialData.Material.StandardShaderRenderingMode.Opaque;
                        materialData.material.metallic = pair.Value.materialWeight;
                        materialData.material.smoothness = 1f - pair.Value.GetNormalizedPropertyValues(1);   //bit(1) : Roughness
                        break;
                    case VoxelData.VoxMaterial.Type.glass:
                        ...
                        materialData.material.alpha = 1f - pair.Value.materialWeight;
                        break;
                    case VoxelData.VoxMaterial.Type.emissive:
                        materialData.material.renderingMode = MaterialData.Material.StandardShaderRenderingMode.Opaque;
                        materialData.material.emission = Color.Lerp(Color.black, Color.white, pair.Value.materialWeight);
                        materialData.material.emissionPower = Mathf.Lerp(2f, 12f, pair.Value.GetNormalizedPropertyValues(5) / 4f);   //bit(5) : Power
                        break;
                    }
                    for (int i = 0; i < voxels.Length; i++)
                    {
                        if(voxels[i].palette == pair.Key)
                        {
        ...
        protected bool LoadVoxelDataFromPNG(BinaryReader br)
        {
            ...
            IntVector3 voxelSize = new IntVector3(Mathf.FloorToInt(rect.width), Mathf.FloorToInt(rect.height), 1);
            VoxelData.Voxel[] voxels;
            Color[] palettes;
            {
                List<VoxelData.Voxel> voxelList = new List<VoxelData.Voxel>(Mathf.FloorToInt(rect.width) * Mathf.FloorToInt(rect.height));
                Dictionary<Color, int> paletteList = new Dictionary<Color, int>();
                for (int x = 0; x < rect.width; x++)
                {
                    for (int y = 0; y < rect.height; y++)
                    {
                        var color = tex.GetPixel(x + Mathf.FloorToInt(rect.xMin), y + Mathf.FloorToInt(rect.yMin));
        ...
        protected bool CreateTexture_LowTexture(Color[] palettes, ref AtlasRectTable atlasRectTable, ref Texture2D atlasTexture, ref Rect[] atlasRects)
        {
            ...
            {
                Texture2D tex;
                if (atlasTexture == null || IsMainAsset(atlasTexture))
                {
        ...
        {
            if (material.shader != Shader.Find("Standard")) return;
            ...
            case MaterialData.Material.StandardShaderRenderingMode.Opaque:
                ...
                {
                    if ((material.globalIlluminationFlags & (MaterialGlobalIlluminationFlags.RealtimeEmissive | MaterialGlobalIlluminationFlags.BakedEmissive)) == 0)
                ...
                break;
            case MaterialData.Material.StandardShaderRenderingMode.Cutout:
                ...
                material.color = Color.white;
                break;
            case MaterialData.Material.StandardShaderRenderingMode.Fade:
        ...
        #region Asset
        public bool IsMainAsset(UnityEngine.Object obj)
        {
            return (obj != null && AssetDatabase.Contains(obj) && AssetDatabase.IsMainAsset(obj));
        }
        public bool IsSubAsset(UnityEngine.Object obj)
        {
        ...
        public void DestroyUnusedObjectInPrefabObject()
        {
            ...
            if (prefabType == PrefabType.Prefab)
                ...
                path = AssetDatabase.GetAssetPath(PrefabUtility.GetPrefabParent(voxelBase.gameObject));
            else
                return;
            ...
            foreach (var obj in objects)
            {
                ...
                if (voxelBase.IsUseAssetObject(obj)) continue;
                #region Extra
                {
                    var explosion = voxelBase.GetComponent<VoxelBaseExplosion>();
                    if(explosion != null && explosion.IsUseAssetObject(obj)) continue;
                }
                #endregion
        ...
        #region PrefabCreatedGameObject
        protected static List<GameObject> prefabCreatedGameObject;
        public static void PrefabCreatedGameObjectAdd(GameObject go)
        {
            if (prefabCreatedGameObject == null)
                prefabCreatedGameObject = new List<GameObject>();
            prefabCreatedGameObject.Add(go);
        }
        public static void PrefabCreatedGameObjectRemove(GameObject go)
        {
            if (prefabCreatedGameObject == null) return;
            for (int i = 0; i < prefabCreatedGameObject.Count; i++)
            {
                if (prefabCreatedGameObject[i] == null)
                {
                ...
                {
                    if (PrefabUtility.GetPrefabParent(prefabCreatedGameObject[i]) == go)
                    {
                        prefabCreatedGameObject.RemoveAt(i--);
                        continue;
                    }
        ...
        public static GameObject PrefabCreatedGameObjectContains(GameObject go)
        {
            if (prefabCreatedGameObject == null) return null;
            for (int i = 0; i < prefabCreatedGameObject.Count; i++)
            {
                if (prefabCreatedGameObject[i] == null) continue;
                var prefabType = PrefabUtility.GetPrefabType(prefabCreatedGameObject[i]);
                if(prefabType == PrefabType.PrefabInstance || prefabType == PrefabType.DisconnectedPrefabInstance)
                {
                    if (PrefabUtility.GetPrefabParent(prefabCreatedGameObject[i]) == go)
        ...
        public bool ExportDaeFile(string path)
        {
