...
namespace VoxelImporter
{
    public class VoxelHumanoidConfigreAvatar : EditorWindow
    {
        ...
        private void Initialize(VoxelSkinnedAnimationObject animationObject)
        {
            ...
            avatarIcon = EditorCommon.EditorGUIUtilityLoadIcon("avatar icon");
        ...
        void OnGUI()
        {
...
#else
                guiStyleBackgroundBox = new GUIStyle("AnimationCurveEditorBackground");
#endif
            }
            ...
            #region HumanoidAvatarConfigreMode
            {
                ...
                humanoidAvatarConfigreMode = (HumanoidAvatarConfigreMode)GUI.Toolbar(EditorGUILayout.GetControlRect(), (int)humanoidAvatarConfigreMode, HumanoidAvatarConfigreModeStrings);
            ...
            if (humanoidAvatarConfigreMode == HumanoidAvatarConfigreMode.Mapping)
            {
...
#else
                            GUI_PreviewBone(new Vector2(position.x, 195), true, VoxelSkinnedAnimationObject.HumanoidBone.Spine);
                            GUI_PreviewBone(new Vector2(position.x, 161), false, VoxelSkinnedAnimationObject.HumanoidBone.Chest);
#endif
                ...
                #region Common Menu
                {
                    ...
                    #region Mapping
                    rect.position = new Vector2(rect.position.x + 6, rect.position.y + 1);
                    rect.width = 64;
                    if (GUI.Button(rect, "Mapping", guiStyleGVGizmoDropDown))
                    {
                    ...
                    rect.position = new Vector2(rect.position.x + rect.width, rect.position.y);
                    rect.width = 48;
                    if (GUI.Button(rect, "Pose", guiStyleGVGizmoDropDown))
                    {
                        GenericMenu menu = new GenericMenu();
            ...
            else if (humanoidAvatarConfigreMode == HumanoidAvatarConfigreMode.MusclesAndSettings)
            {
                ...
                #region Common Menu
                {
                    ...
                    rect.position = new Vector2(rect.position.x + 6, rect.position.y + 1);
                    rect.width = 64;
                    if (GUI.Button(rect, "Muscles", guiStyleGVGizmoDropDown))
                    {
                        GenericMenu menu = new GenericMenu();
            ...
            #region Button
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    #region Revert
                    rect.position = new Vector2(rect.position.x + rect.width - 150, rect.position.y + 16);
                    rect.width = 54;
                    if (GUI.Button(rect, "Revert"))
                    {
                    ...
                    #region Apply
                    rect.position = new Vector2(rect.position.x + rect.width + 4, rect.position.y);
                    rect.width = 44;
        ...
        private bool IsBoneTransformParentError(VoxelSkinnedAnimationObject.HumanoidBone boneIndex, ref string errorMessage)
        {
            ...
            case VoxelSkinnedAnimationObject.HumanoidBone.LeftShoulder:
                ...
                if (!CheckTransform(VoxelSkinnedAnimationObject.HumanoidBone.UpperChest) &&
                    ...
                    result = false;
#else
                if (!CheckTransform(VoxelSkinnedAnimationObject.HumanoidBone.Chest) &&
                    !CheckTransform(VoxelSkinnedAnimationObject.HumanoidBone.Spine))
                    result = false;
#endif
                break;
